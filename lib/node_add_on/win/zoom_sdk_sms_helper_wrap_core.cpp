...
public:
	...
	virtual void onNeedRealNameAuthMeetingNotification(const zTCHAR* bind_phone_url, const zTCHAR* signup_url)
	{
		if (owner_) 
		{
			owner_->onNeedRealNameAuthMeetingNotification(bind_phone_url, signup_url);
		}
	}
	virtual void onNeedRealNameAuthMeetingNotification(IVector<IZoomRealNameAuthCountryInfo* >* support_country_list, const zTCHAR* privacy_url, IZoomRetrieveSMSVerificationCodeHandler* handler)
	{
		if (owner_ && support_country_list && handler)
		{
			...
			for (int i = 0; i < support_country_list->GetCount(); ++i)
			{
				...
				{
					countryInfo.countryID = temp_pCountryInfo->GetCountryID();
					countryInfo.countryName = temp_pCountryInfo->GetCountryName();
					countryInfo.countryCode = temp_pCountryInfo->GetCountryCode();
				}
				SMSCountryInfo_list.push_back(countryInfo);
			}
			owner_->onNeedRealNameAuthMeetingNotification(SMSCountryInfo_list, privacy_url);
		}
	}
...
{
	bool b_ret = false;
	b_ret = ZoomRetrieveSMSVerificationCodeHandler::GetInst().Retrieve(wStr2Str(country_code).c_str(), wStr2Str(phone_number).c_str());
	return b_ret;
}
...
{
	bool b_ret = false;
	b_ret = ZoomVerifySMSVerificationCodeHandler::GetInst().Verify(wStr2Str(country_code).c_str(), wStr2Str(phone_number).c_str(), wStr2Str(verification_code).c_str());
	return b_ret;
}
...
void ZSDKSMSHelperWrap::onNeedRealNameAuthMeetingNotification(std::string bind_phone_url, std::string signup_url)
{
	if (m_pSink)
	{
		m_pSink->onNeedRealNameAuthMeetingNotification(bind_phone_url, signup_url);
	}
}
