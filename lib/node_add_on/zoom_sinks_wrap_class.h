...
{
public:
	virtual void onNeedRealNameAuthMeetingNotification(std::string bind_phone_url, std::string signup_url)
	{
		if (ZoomNodeSinkHelper::GetInst().onNeedRealNameAuthMeetingNotification_withoutCountryList.IsEmpty())
			return;
		...
		v8::Local<v8::String> zn_bind_phone_url = v8::String::NewFromUtf8(isolate, bind_phone_url.c_str());
		v8::Local<v8::String> zn_signup_url = v8::String::NewFromUtf8(isolate, signup_url.c_str());

		int argc = 2;
		v8::Local<v8::Value> argv[2] = { zn_bind_phone_url, zn_signup_url };
		auto fn = v8::Local<v8::Function>::New(isolate, ZoomNodeSinkHelper::GetInst().onNeedRealNameAuthMeetingNotification_withoutCountryList);

		fn->Call(context, global, argc, argv);
	}
	virtual void onNeedRealNameAuthMeetingNotification(ZNList<ZNZoomRealNameAuthCountryInfo> support_country_list, std::string privacy_url)
	{
		...
		for (unsigned int i = 0; i < support_country_list.size(); ++i)
		{
			v8::HandleScope scope(isolate);
			v8::Local<v8::Object> node = v8::Object::New(isolate);
			node->Set(v8::String::NewFromUtf8(isolate, "countryCode"), v8::String::NewFromUtf8(isolate, support_country_list[i].countryCode.c_str()));
			node->Set(v8::String::NewFromUtf8(isolate, "countryID"), v8::String::NewFromUtf8(isolate, support_country_list[i].countryID.c_str()));
			node->Set(v8::String::NewFromUtf8(isolate, "countryName"), v8::String::NewFromUtf8(isolate, support_country_list[i].countryName.c_str()));
			zn_support_country_list->Set(i, node);
		}
		
		v8::Local<v8::String> zn_privacy_url = v8::String::NewFromUtf8(isolate, privacy_url.c_str());
