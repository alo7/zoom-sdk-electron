...
void ZoomNodeSDKSMSHelperWrap::EnableZoomAuthRealNameMeetingUIShown(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	bool zn_enable;
	zoom_v8toc(args[1].As<v8::Boolean>(), zn_enable);
...
void ZoomNodeSDKSMSHelperWrap::GetSupportPhoneNumberCountryList(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	for (unsigned int i = 0; i < zn_support_phone_number_country_lst.size(); ++i) 
	{
		...
		node->Set(v8::String::NewFromUtf8(isolate, "countryCode"), v8::String::NewFromUtf8(isolate, zn_support_phone_number_country_lst[i].countryCode.c_str()));
		node->Set(v8::String::NewFromUtf8(isolate, "countryID"), v8::String::NewFromUtf8(isolate, zn_support_phone_number_country_lst[i].countryID.c_str()));
		node->Set(v8::String::NewFromUtf8(isolate, "countryName"), v8::String::NewFromUtf8(isolate, zn_support_phone_number_country_lst[i].countryName.c_str()));
		nodes->Set(i, node);
	}
	args.GetReturnValue().Set(nodes);
}
void ZoomNodeSDKSMSHelperWrap::SetNeedRealNameAuthMeetingNotificationCB_WithoutCountryList(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	if (args.Length() < 1) {
		isolate->ThrowException(v8::Exception::TypeError(
	...
	ZoomNodeSinkHelper::GetInst().onNeedRealNameAuthMeetingNotification_withoutCountryList = cb;
